...
const targetStr = `  const resetGame = useCallback((targetPlayerColor?: ChessColor) => {
...
let replacement = `  const resetGame = useCallback((targetPlayerColor?: ChessColor) => {
    const color = targetPlayerColor || playerColor;
    setBoard(INITIAL_BOARD);
    setTurn(color === 'black' ? 'black' : 'red'); 
    setSelected(null);
  ...
  const executeMove = useCallback((from, to, currentBoard = board, currentTurn = turn) => {
  ...
  const triggerAiMove = useCallback((currentBoard, currentTurn) => {
