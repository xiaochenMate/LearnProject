...
export class GoEngine {
     ...
     */
    validateMove(board, r, c, color, lastBoard) {
        if (board[r][c] !== 0)
            return null;
    ...
    estimateScore(board, komi = 7.5) {
        ...
        for (let r = 0; r < size; r++) {
            for (let c = 0; c < size; c++) {
                if (board[r][c] === 1 || territory[r][c] === 11)
                    bScore++;
                if (board[r][c] === 2 || territory[r][c] === 12)
                    wScore++;
            }
        }
        return { black: bScore, white: wScore, territory };
    }
    getBestMove(board, color, lastBoard) {
        const validMoves = [];
        const opponent = color === 1 ? 2 : 1;
        for (let r = 0; r < this.size; r++) {
            for (let c = 0; c < this.size; c++) {
                ...
                if ((minDR === 2 || minDR === 3) && (minDC >= 2))
                    score += 30;
                if ((minDC === 2 || minDC === 3) && (minDR >= 2))
                    score += 30;
